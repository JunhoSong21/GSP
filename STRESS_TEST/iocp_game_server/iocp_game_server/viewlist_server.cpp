...
{
	auto old_view = m_visible_players;
	...
	for (auto& cl : clients) {
		...
		if (is_visible(pl->m_x, pl->m_y))
			new_view.insert(pl->m_id);
	}

	send_move_packet(m_id);

	for (int id : new_view) {
		if (old_view.count(id) == 0) {
			send_add_player(id);
			std::shared_ptr<SESSION> pl = clients[m_id].load();
			if (nullptr == pl) continue;
			pl->send_remove_player(id);
		}
	}

	for (int id : old_view) {
		if (new_view.count(id) == 0) {
