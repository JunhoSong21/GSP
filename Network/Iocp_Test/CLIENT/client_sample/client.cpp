...
#include "..\..\Server_Iocp_Test\Protocol.h"
...
void ProcessPacket(char* ptr)
{
	...
	case S2C_ADD_PLAYER:
	{
		S2C_ADD_PLAYER_PACKET* my_packet = reinterpret_cast<S2C_ADD_PLAYER_PACKET*>(ptr);
		int id = my_packet->id;
		if (id >= MAX_USER) {
	...
	{
		S2C_MOVE_PLAYER_PACKET* my_packet = reinterpret_cast<S2C_MOVE_PLAYER_PACKET*>(ptr);
	...
	{
		S2C_REMOVE_PLAYER_PACKET* my_packet = reinterpret_cast<S2C_REMOVE_PLAYER_PACKET*>(ptr);
		int other_id = my_packet->id;
		if (other_id == g_myid)
			avatar.hide();
		else if (other_id < MAX_USER)
